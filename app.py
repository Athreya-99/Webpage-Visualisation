...
def get_medicine_heatmap():
    ...
    return single_medicine_heatmap(patient_id, medicine_name)

def single_medicine_heatmap(patient_id, medicine_name, history_path='history_new.json', alarm_path='alarm_new.json'):
    TIME_TOLERANCE = timedelta(minutes=30)
    EXPECTED_INTERVAL = timedelta(hours=6)
    if not os.path.exists(history_path):
        abort(500, description=f"{history_path} not found")
    with open(history_path) as f:
        history_data = json.load(f)
    history_df = pd.DataFrame(history_data)
    history_df = history_df[history_df["patient_id"] == patient_id]
    if not os.path.exists(alarm_path):
        abort(500, description=f"{alarm_path} not found")
